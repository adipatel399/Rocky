...
- **Screen awareness** — "Jarvis, what am I looking at?" → `screencapture` →
...
- **Menu-bar app** — package the HUD in a tiny wrapper (Tauri is lightest) so
  ...
  a LaunchAgent.
- **Interrupt by voice** — detect "jarvis stop" *while* he's speaking (keep
...
- **Long-term memory** — a `~/.jarvis/memory.md` the brain reads at session
...
- **Browser hands** — add a browser MCP server (e.g. Playwright MCP) to
  ...
  automatically, so Jarvis gains "book the ticket" web skills for free.
...
- **Proactive Jarvis** — file-system and calendar watchers that let him speak
